...
- **General & Group Expenses (Phase I)**: Single-screen fast operational and overhead recording supporting all active companies via an entity book dropdown (defaulting to 7 Orbit holding book), supporting staff tea/kitchen, shared utilities, internet, stationery, and shared fixed assets with automatic GL voucher generation.
- **Quick Income & Fund Receipts (Phase K)**: Dedicated fast operational and capital inflow voucher creation (`QuickIncomeEntryPage`) with smart category defaults (`IncomeCategory`), automatic GL and Treasury routing for customer AR receipts, customer advances, direct construction/service revenue, rental income, director capital, and director loan fund injections into cash or company bank accounts, backed by `QuickIncomeStatsWidget` and real-time receipts audit ledger.
- **General Group Asset Registry & Cross-Company Custody Hub (Phase J)**: Central group physical asset ownership (`7 Orbit Corporate` holding book) with cross-company operational deployment (`BMC Construction`, `7 Orbit Medical Billing`, `YMC Construction`) and individual employee custody accountability, custody lifecycle (`InPool`, `AssignedToCompany`, `AssignedToEmployee`), dynamic modal re-assignments, return-to-storage pool actions, and automated HR employee exit clearance verification.
...
### Department Hierarchy, Teams, Task Management & Daily Work Reporting

- **Department & Team Hierarchy**:
  - Structure: Company $\rightarrow$ Department $\rightarrow$ Department Teams $\rightarrow$ Team Leads $\rightarrow$ Team Members $\rightarrow$ Tasks $\rightarrow$ Daily Work Reports $\rightarrow$ 2-Tier Approvals $\rightarrow$ Productivity Analytics.
  - Team types: Social Media, Graphic Design, Video Production, Web Development, Sales / Business Development, General.
  - Pivot table `department_team_members` with roles (`lead`, `member`) and date tracking.
- **Task Management System**:
  - Atomic sequence generation: `TSK-YYYY-XXXXX`.
  - Task priorities (`Low`, `Medium`, `High`, `Urgent`), statuses (`NotStarted`, `InProgress`, `Submitted`, `RevisionRequired`, `Approved`, `Completed`, `Cancelled`).
  - Progress percentage, estimated hours, actual hours, deadline date, blocker flag & reason.
  - Specialized 1:1 workflow tables: `social_media_task_details`, `design_task_details`, `video_task_details`, `web_dev_task_details`.
  - Polymorphic/secure Task Attachments & Deliverable URLs (Figma, Canva, Drive, Loom, GitHub) with versioning.
  - Task Revisions history with requester, timestamp, and feedback notes.
  - Task Comments for team collaboration and blocker reporting.
- **2-Tier Approval Chain**:
  - Step 1: Team Member Submits deliverable $\rightarrow$ Status: `Submitted`.
  - Step 2: Team Lead Review $\rightarrow$ Lead approves (passes to Head) or requests revision.
  - Step 3: Department Head Final Approval $\rightarrow$ Final signoff, marks `Completed`, records completion timestamp.
- **Daily Work Reporting & 6:00 PM Deadline**:
  - Daily check-in submissions with automatic 6:00 PM cutoff evaluation (`OnTime` $\le$ 18:00, `Late` > 18:00).
  - Itemized task breakdown with hours spent, progress %, deliverable summary, and blocker notes.
  - Specialized daily sales metrics detail (`leads_received`, `contacted`, `calls_made`, `meetings_booked`, `proposals_sent`, `deals_closed`, `revenue_generated`).
  - `work-reports:check-deadline` console command for automated 6:00 PM missing submission notifications.
- **Dashboards & Visual Matrix**:
  - **Head Operations Dashboard**: KPI cards, Awaiting Approval Queue with 1-click actions, Team-wise performance matrix.
  - **Daily Attendance & Work Matrix Page**: Color-coded real-time grid (🟢 Green = On Time, 🟠 Orange = Late, 🔴 Red = Missing) with date selector and team filters.
  - **Employee Performance & Productivity Page**: Auto-calculates turnaround hours, task completion rate, revision rate, on-time report rate, and 0–100 composite productivity index.
