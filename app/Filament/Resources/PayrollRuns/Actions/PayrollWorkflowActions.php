...
class PayrollWorkflowActions
{
    public static function generate(): Action
    {
        return Action::make('generateEntries')->label('Generate / Refresh Entries')->icon('heroicon-o-calculator')
            ...
            ->requiresConfirmation()->action(fn (PayrollRun $record, GeneratePayrollEntriesAction $generateEntriesAction) => self::run($record, 'generateEntries', fn () => $generateEntriesAction->handle($record, self::user()), 'Payroll entries generated'));
    }
    ...
    public static function submit(): Action
    {
        return Action::make('submit')->label('Submit for Review')->color('warning')->icon('heroicon-o-paper-airplane')
            ...
            ->requiresConfirmation()->action(fn (PayrollRun $record, SubmitPayrollRunAction $submitRunAction) => self::run($record, 'submit', fn () => $submitRunAction->handle($record, self::user()), 'Payroll submitted'));
    }
    ...
    public static function approve(): Action
    {
        return Action::make('approve')->color('success')->icon('heroicon-o-check-badge')->authorize('approve')
            ...
            ->action(fn (PayrollRun $record, ApprovePayrollRunAction $approveRunAction) => self::run($record, 'approve', fn () => $approveRunAction->handle($record, self::user()), 'Payroll approved'));
    }
    ...
    public static function reject(): Action
    {
        return Action::make('reject')->color('danger')->icon('heroicon-o-x-circle')->authorize('reject')
            ...
            ->action(function (array $data, PayrollRun $record, RejectPayrollRunAction $rejectRunAction): void {
                Gate::authorize('reject', $record);
                $rejectRunAction->handle($record, self::user(), $data['reason']);
    ...
    public static function markPaid(): Action
    {
        return Action::make('markPaid')->label('Mark Paid')->color('info')->icon('heroicon-o-banknotes')->authorize('markPaid')
            ...
            ->action(fn (PayrollRun $record, MarkPayrollRunPaidAction $markPaidAction) => self::run($record, 'markPaid', fn () => $markPaidAction->handle($record, self::user()), 'Payroll marked paid'));
    }
    ...
    public static function post(): Action
    {
        return Action::make('post')->label('Post to Accounts')->color('success')->icon('heroicon-o-book-open')
            ...
            ->action(fn (PayrollRun $record, PostPayrollRunAction $postRunAction) => self::run(
                $record, 'post', fn () => $postRunAction->handle($record, self::user()), 'Payroll posted to Accounts',
            ));
    }
    ...
    public static function reverse(): Action
    {
        return Action::make('reverse')->label('Reverse Posting')->color('danger')->icon('heroicon-o-arrow-uturn-left')
            ...
            ])->action(function (array $data, PayrollRun $record, ReversePayrollRunAction $reverseRunAction): void {
                Gate::authorize('reverse', $record);
                $reverseRunAction->handle($record, self::user(), Carbon::parse($data['reversal_date']), $data['reason']);
    ...
    public static function lock(): Action
    {
        return Action::make('lock')->label('Lock Payroll')->color('primary')->icon('heroicon-o-lock-closed')->authorize('lock')
            ...
            ->action(fn (PayrollRun $record, LockPayrollRunAction $lockRunAction) => self::run($record, 'lock', fn () => $lockRunAction->handle($record, self::user()), 'Payroll locked'));
    }
